  ...
  return (
    <ol className="flex flex-col gap-1.5">
      {stages.map((s, i) => {
        ...
        const width = Math.min(60, Math.max(4, (Math.log10(s.volume + 1) / maxLog) * 60));
        return (
          <li key={s.stage} className={cn("grid grid-cols-[7rem_1fr] items-center gap-3", !compact && "md:grid-cols-[8rem_1fr_9rem_8rem_8rem]")}>
              ...
              <>
                <div className="tnum text-xs text-muted">
                  {s.conversionFromPrevious !== null ? (
                    <>
                      <span className="text-foreground">{fmtPct(s.conversionFromPrevious, s.conversionFromPrevious < 0.01 ? 2 : 1)}</span>{" "}
                      {s.stage === "revenue" ? "win rate" : "conv."}
                    </>
                  ) : (
                    "—"
                  )}
                ...
                <div className="tnum text-xs text-muted">
                  {s.costPer !== null ? (
                    <>
                      <span className="text-foreground">{fmtCurrency(s.costPer, { cents: s.costPer < 10 })}</span> each
                    </>
                  ) : (
                    "—"
                  )}
                ...
                <div className="tnum text-xs text-muted">
                  {s.value !== null ? (
                    <>
                      <span className="text-foreground">{fmtCompactCurrency(s.value)}</span> value
                    </>
                  ) : (
                    "—"
                  )}
